...
KEYWORDS = [
    ...
    # Контент
    "рилс ai", "reels ai", "tiktok ai", "shorts ai",
    "клип нейросеть", "видео реклама ai", "ai контент",
    # Английские короткие
...
def run_hunter_scheduler():
    ...
    while True:
        ...
        # Переназначаем расписание если изменился интервал
        if interval != last_interval:
            # Удаляем старые задачи hunter
            schedule.get_jobs()
            for job in schedule.get_jobs():
                if job.job_func == hunter_job:
                    schedule.cancel_job(job)
            schedule.every(interval).minutes.do(hunter_job)
            last_interval = interval
            log.info(f"🔍 Hunter расписание: каждые {interval} мин")
        time.sleep(60)
        schedule.run_pending()
